...
const CandidateRegistration = () => {
  ...
  const addSkill = (type: 'primary' | 'secondary') => {
    ...
    if (input.trim() && !formData[field].includes(input.trim())) {
      ...
      type === 'primary' ? setPrimarySkillInput('') : setSecondarySkillInput('');
    }
  };

  const removeSkill = (skill: string, type: 'primary' | 'secondary') => {
    const field = type === 'primary' ? 'primarySkills' : 'secondarySkills';
    updateFormData(field, formData[field].filter(s => s !== skill));
  };
    ...
    if (file) {
      ...
      if (validTypes.includes(file.type)) {
        ...
        toast({
          ...
          description: "Your resume has been uploaded successfully. AI will extract skills automatically.",
        });
      } else {
        toast({
    ...
    switch (step) {
      ...
      case 2:
        if (!formData.candidateType) {
          toast({ title: "Please select a candidate type", variant: "destructive" });
          return false;
        }
        return true;
      case 3:
        if (!formData.primaryJobRole || !formData.totalExperience || !formData.currentCity || !formData.currentCountry || !formData.preferredWorkType) {
          toast({ title: "Please fill all required fields", variant: "destructive" });
          return false;
        }
        return true;
      case 4:
        if (formData.primarySkills.length === 0) {
          toast({ title: "Please add at least one primary skill", variant: "destructive" });
          return false;
        }
        return true;
      case 5:
      ...
      case 6:
        if (!formData.acceptTerms || !formData.acceptPrivacy) {
          toast({ title: "Please accept Terms & Conditions and Privacy Policy", variant: "destructive" });
          return false;
        }
        return true;
      default:
  ...
  const handleSubmit = () => {
    toast({
      ...
      description: "Your profile has been created. Please verify your email to complete registration.",
    });
  ...
  const renderStep1 = () => (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Enter your personal details to begin your journey with Hirion.</p>
      </div>
  ...
  const renderStep2 = () => (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-foreground">What type of candidate are you?</h2>
        <p className="text-muted-foreground mt-1">This helps us personalize your job recommendations.</p>
      </div>
      ...
      >
        ...
        >
          ...
          <div className="flex-1">
            <Label htmlFor="fulltime" className="text-lg font-semibold cursor-pointer">
        ...
        >
          ...
          <div className="flex-1">
            <Label htmlFor="contract" className="text-lg font-semibold cursor-pointer">
  ...
  const renderStep3 = () => (
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        <Label htmlFor="primaryJobRole">Primary Job Role <span className="text-destructive">*</span></Label>
        <Input
          id="primaryJobRole"
      ...
      <div className="space-y-2">
        <Label>Total Experience <span className="text-destructive">*</span></Label>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          <Label htmlFor="currentCity">City <span className="text-destructive">*</span></Label>
          <Input
            id="currentCity"
        ...
        <div className="space-y-2">
          <Label htmlFor="currentCountry">Country <span className="text-destructive">*</span></Label>
          <Input
            id="currentCountry"
      ...
      <div className="space-y-2">
        <Label>Preferred Work Type <span className="text-destructive">*</span></Label>
        <Select value={formData.preferredWorkType} onValueChange={(value) => updateFormData('preferredWorkType', value)}>
          <SelectTrigger>
  ...
  const renderStep4 = () => (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Add skills that showcase your expertise.</p>
      </div>

      <div className="space-y-4">
        <div className="space-y-2">
          <Label>Primary Skills <span className="text-destructive">*</span></Label>
          <div className="flex gap-2">
            <Input
              placeholder="Add a skill (e.g., React, Python)"
              value={primarySkillInput}
              onChange={(e) => setPrimarySkillInput(e.target.value)}
              onKeyPress={(e) => e.key === 'Enter' && (e.preventDefault(), addSkill('primary'))}
            />
            <Button type="button" onClick={() => addSkill('primary')}>Add</Button>
          </div>
          <div className="flex flex-wrap gap-2 mt-2">
            {formData.primarySkills.map(skill => (
              <Badge key={skill} variant="default" className="gap-1 py-1.5 px-3">
                {skill}
                <X className="h-3 w-3 cursor-pointer ml-1" onClick={() => removeSkill(skill, 'primary')} />
              </Badge>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-2">
          <Label>Secondary Skills <span className="text-muted-foreground">(Optional)</span></Label>
          <div className="flex gap-2">
            <Input
              placeholder="Add secondary skills"
              value={secondarySkillInput}
              onChange={(e) => setSecondarySkillInput(e.target.value)}
              onKeyPress={(e) => e.key === 'Enter' && (e.preventDefault(), addSkill('secondary'))}
            />
            <Button type="button" variant="outline" onClick={() => addSkill('secondary')}>Add</Button>
          </div>
          <div className="flex flex-wrap gap-2 mt-2">
            {formData.secondarySkills.map(skill => (
              <Badge key={skill} variant="secondary" className="gap-1 py-1.5 px-3">
                {skill}
                <X className="h-3 w-3 cursor-pointer ml-1" onClick={() => removeSkill(skill, 'secondary')} />
              </Badge>
            ))}
          </div>
        </div>
  ...
  const renderStep5 = () => (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Our AI will automatically extract your skills and experience.</p>
      </div>

      <div 
        className={`border-2 border-dashed rounded-xl p-8 text-center transition-all ${
          formData.resumeFile ? 'border-primary bg-primary/5' : 'border-border hover:border-primary/50'
        }`}
      >
        <input
          type="file"
        ...
        <label htmlFor="resume" className="cursor-pointer">
          {formData.resumeFile ? (
            <div className="space-y-3">
              ...
              <div>
                <p className="font-semibold text-foreground">{formData.resumeFile.name}</p>
      ...
      <div className="bg-primary/10 p-4 rounded-xl flex items-start gap-3">
        ...
        <div>
          <p className="font-medium text-foreground">AI-Powered Skill Extraction</p>
          <p className="text-sm text-muted-foreground">
            Our AI will automatically identify and extract skills from your resume to improve your profile visibility.
          </p>
        </div>
      </div>
    </div>
  ...
  const renderStep6 = () => (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground mt-1">Set your preferences for better job matches. (Optional fields)</p>
      </div>
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            {formData.preferredLocations.map(location => (
              <Badge key={location} variant="secondary" className="gap-1 py-1.5 px-3">
                {location}
                <X className="h-3 w-3 cursor-pointer ml-1" onClick={() => removeLocation(location)} />
              </Badge>
            ))}
          </div>
        </div>
        ...
        <div className="space-y-2">
          <Label>Available to Join <span className="text-destructive">*</span></Label>
          <Select value={formData.availableToJoin} onValueChange={(value) => updateFormData('availableToJoin', value)}>
            <SelectTrigger>
        ...
        <div className="space-y-3">
          <div className="flex items-start gap-3">
            ...
            <Label htmlFor="terms" className="text-sm cursor-pointer">
              I accept the <Link to="#" className="text-primary hover:underline">Terms & Conditions</Link> <span className="text-destructive">*</span>
            </Label>
          </div>
          ...
          <div className="flex items-start gap-3">
            ...
            <Label htmlFor="privacy" className="text-sm cursor-pointer">
              I accept the <Link to="#" className="text-primary hover:underline">Privacy Policy</Link> <span className="text-destructive">*</span>
            </Label>
          </div>
          ...
          <div className="flex items-start gap-3">
            <Checkbox
              ...
              onCheckedChange={(checked) => updateFormData('takeAssessment', checked)}
            />
          ...
          <div className="flex items-start gap-3">
            <Checkbox
              ...
              onCheckedChange={(checked) => updateFormData('scheduleInterview', checked)}
            />
      ...
      <main className="flex-1 py-8 px-4">
        <div className="container mx-auto max-w-3xl">
          ...
          <p className="text-center mt-6 text-muted-foreground">
            ...
            <Link to="/candidate-login" className="text-primary font-medium hover:underline">
              Log in here
            </Link>
          </p>
        </div>
