...
const CreateJob = () => {
  ...
  const removeSkill = (index: number) => {
    updateFormData('requiredSkills', formData.requiredSkills.filter((_, i) => i !== index));
  };
  ...
  const addNiceToHave = () => {
    if (niceToHaveInput.trim() && !formData.niceToHaveSkills.includes(niceToHaveInput.trim())) {
      updateFormData('niceToHaveSkills', [...formData.niceToHaveSkills, niceToHaveInput.trim()]);
      setNiceToHaveInput('');
    }
  };
...
const handlePublish = async () => {
      ...
      <CardContent className="space-y-5">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={formData.experienceLevel} onValueChange={(v) => updateFormData('experienceLevel', v)}>
              <SelectTrigger>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="space-y-2">
            ...
            <Select value={formData.workLocation} onValueChange={(v) => updateFormData('workLocation', v)}>
              <SelectTrigger>
  ...
  const renderSkillsRequirements = () => (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Define the skills needed for this role</CardDescription>
      </CardHeader>
            ...
            <div className="space-y-2 mt-3">
              {formData.requiredSkills.map((skill, index) => (
                <div key={index} className="flex items-center gap-3 p-3 bg-muted/50 rounded-lg">
                  <Badge className="bg-primary text-primary-foreground">{skill.name}</Badge>
                  <Select value={skill.level} onValueChange={(v) => updateSkillLevel(index, v)}>
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            <Input
              ...
              className="flex-1"
            />
            <Button type="button" onClick={addNiceToHave} variant="outline">Add</Button>
          </div>
              ...
              {formData.niceToHaveSkills.map((skill, index) => (
                <Badge key={index} variant="secondary" className="gap-1 py-1.5">
                  ...
                  <X 
                    ...
                    onClick={() => updateFormData('niceToHaveSkills', formData.niceToHaveSkills.filter((_, i) => i !== index))}
                  />
                </Badge>
              ))}
            </div>
  ...
  const renderSkillValidation = () => (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Configure technical assessments for candidates</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="flex items-center justify-between p-4 bg-muted/50 rounded-lg">
          <div className="space-y-0.5">
            <Label className="text-base font-medium">Enable Skill Test</Label>
            <p className="text-sm text-muted-foreground">Automatically test candidates' technical skills</p>
          </div>
          <Switch
            checked={formData.enableSkillTest}
            onCheckedChange={(checked) => updateFormData('enableSkillTest', checked)}
          />
        </div>
          ...
          <div className="space-y-5 animate-in fade-in-50 slide-in-from-top-2">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Select value={formData.testType} onValueChange={(v) => updateFormData('testType', v)}>
                  <SelectTrigger>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-medium">{formData.autoRejectScore}%</span>
              </div>
              <Slider
  ...
  const renderAIInterview = () => (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Configure AI-powered candidate interviews</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        <div className="flex items-center justify-between p-4 bg-muted/50 rounded-lg">
          <div className="space-y-0.5">
            <Label className="text-base font-medium">Enable AI Interview</Label>
            <p className="text-sm text-muted-foreground">Conduct automated interviews with AI evaluation</p>
          </div>
          <Switch
            checked={formData.enableAIInterview}
            onCheckedChange={(checked) => updateFormData('enableAIInterview', checked)}
          />
        </div>
        ...
        {formData.enableAIInterview && (
          <div className="space-y-5 animate-in fade-in-50 slide-in-from-top-2">
            <div className="space-y-2">
              ...
              <Select value={formData.interviewType} onValueChange={(v) => updateFormData('interviewType', v)}>
                <SelectTrigger>
              ...
              <div className="space-y-3">
                <div className="flex items-center gap-3 p-3 bg-muted/30 rounded-lg">
                  ...
                  <Checkbox 
                    ...
                    onCheckedChange={(checked) => updateFormData('evaluateCommunication', checked)}
                  />
                  <div className="flex items-center gap-2">
                    <MessageSquare className="h-4 w-4 text-muted-foreground" />
                    <Label htmlFor="communication" className="cursor-pointer font-normal">Communication Skills</Label>
                  </div>
                </div>
                <div className="flex items-center gap-3 p-3 bg-muted/30 rounded-lg">
                  <Checkbox 
                    ...
                    onCheckedChange={(checked) => updateFormData('evaluateProblemSolving', checked)}
                  />
                  <div className="flex items-center gap-2">
                    <Layers className="h-4 w-4 text-muted-foreground" />
                    <Label htmlFor="problemSolving" className="cursor-pointer font-normal">Problem Solving</Label>
                  </div>
                </div>
                <div className="flex items-center gap-3 p-3 bg-muted/30 rounded-lg">
                  <Checkbox 
                    ...
                    onCheckedChange={(checked) => updateFormData('evaluateTechnicalDepth', checked)}
                  />
                  <div className="flex items-center gap-2">
                    <Code className="h-4 w-4 text-muted-foreground" />
                    <Label htmlFor="technicalDepth" className="cursor-pointer font-normal">Technical Depth</Label>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm font-medium">{formData.autoAdvanceScore}%</span>
              </div>
              <Slider
              ...
              />
              <p className="text-xs text-muted-foreground">
                Candidates scoring above this will be automatically advanced to human interview
              </p>
            </div>
          </div>
        )}
      ...
      <CardContent className="space-y-6">
        <div className="space-y-4">
          <div className="p-4 bg-muted/50 rounded-lg">
            <h3 className="font-semibold text-lg">{formData.jobTitle || 'Untitled Job'}</h3>
            <div className="flex gap-2 mt-2">
              <Badge variant="outline">{formData.employmentType || 'Type not set'}</Badge>
              <Badge variant="outline">{formData.workLocation || 'Location not set'}</Badge>
              <Badge variant="outline">{formData.experienceLevel || 'Level not set'}</Badge>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-4 border rounded-lg">
              ...
              <Badge variant={formData.enableSkillTest ? "default" : "secondary"}>
                {formData.enableSkillTest ? 'Enabled' : 'Disabled'}
              </Badge>
              {formData.enableSkillTest && (
                <p className="text-sm text-muted-foreground mt-1">
                  {formData.testType} • {formData.difficultyLevel} • {formData.timeLimit}min
                </p>
              )}
            </div>
            <div className="p-4 border rounded-lg">
              <h4 className="font-medium mb-2">AI Interview</h4>
              <Badge variant={formData.enableAIInterview ? "default" : "secondary"}>
          ...
          <div className="p-4 border rounded-lg">
            <h4 className="font-medium mb-2">Required Skills ({formData.requiredSkills.length})</h4>
  ...
  const renderCurrentStep = () => {
    switch (currentStep) {
      case 1: return renderJobDetails();
      case 2: return renderSkillsRequirements();
      case 3: return renderSkillValidation();
      case 4: return renderAIInterview();
      case 5: return renderPublish();
  ...
  return (
    <div className="space-y-6 pb-24">
      ...
      <div>
        ...
        <p className="text-muted-foreground">Post a new job with skill tests and AI interviews</p>
      </div>
